...
On Windows, you can also open `.md` and `.mdx` files from Explorer with **Open With → OpenKnowledge**, or double-click them when OpenKnowledge is their default app. With the desktop app installed, these actions open a project or a standalone file using the same rules as `ok <file>`; see [Windows file associations and changing the default app](/docs/reference/what-open-knowledge-writes#installing-and-launching-the-app).
...
Hover over a bullet, numbered, or task-list item and drag its grip to move that item within the list or elsewhere in the document. Nested items have their own grips; select several items or a passage spanning multiple blocks before dragging to move them together. Task checkboxes keep their checked state. Reordering a numbered list's items, or moving items into or out of one, renumbers that list's items sequentially from its starting number, replacing custom gaps or repeated numbers. Moving a whole numbered list preserves its authored numbers unless its items merge into another list.

To reorder with the keyboard, place the cursor in an item or select several neighboring items, then press `Cmd+Shift+↑` or `Cmd+Shift+↓` on macOS (`Ctrl+Shift+↑` or `Ctrl+Shift+↓` on Windows and Linux). At a list's edge, the same shortcut moves the selection past the neighboring block or out of its parent item.
...
Aliased wiki links and embeds must escape their separator inside a table cell: write it as `\|`, since a bare `|` ends the cell. `[[Page\|alias]]` in a cell resolves identically to `[[Page|alias]]` outside one, and the escape you author is preserved in the saved file. Two corners escape that preservation, and only the first is about cells. In a cell of any table rather than only a migrated one, a code span or raw HTML whose own content holds a backslash immediately before a pipe gains a second backslash on save. That extra backslash stops the pipe from reading as an escape, so the cell ends at that pipe and the value is cut in two. OpenKnowledge then shows the tail as an extra cell in that row, or stops parsing the block as a table when the affected cell is in the header row; other readers differ on the same bytes, and some drop the tail instead of showing it — so treat anything after that pipe as at risk rather than merely misplaced. The second corner is not a table phenomenon and does not strike on save: an aliased wiki link or embed whose alias holds that same pair, anywhere in a document, is written to the file with the backslash intact and comes back without it the next time the file is opened. Nothing is cut in two and no row breaks, so the only cue is the alias itself reading `a|b` where you wrote `a\|b`. Neither corner is reachable by writing the source: both arrive by pasting, or by editing the cell or the alias in place. Writing `\|` as an alias separator, or inside a code span in a cell, stays the correct form and is preserved as authored.
...
While spell check is off, right-clicking any editable text offers **Enable Spell Check**, and the menu bar carries the same toggle as **Edit ▸ Check Spelling While Typing** (see [Edit menu](#edit-menu)). You can also change **Check spelling while typing** in **Settings → User → Preferences**, beside **Language**. These controls share the same app-wide preference.

On Windows and Linux, **Spelling languages** lets you check multiple languages together, such as English and Vietnamese, independently of the interface language. The choice applies across projects in the same desktop profile and survives a restart. Keep at least one language selected; use the spelling toggle to turn checking off without losing your languages. On macOS, checking languages remain managed by macOS.

Uncached dictionaries may download from Google's dictionary CDN; document text and typed words are not sent to Google for spell checking ([Electron's spellchecker documentation](https://www.electronjs.org/docs/latest/tutorial/spellchecker#does-the-spellchecker-use-any-google-services)). Downloads need an internet connection, and existing underlines may not update immediately after changing languages.
...
It's the primary in-editor way to hand work to an agent, and the freetext companion to the menu-driven [Open with AI](#open-with-ai) (whole file / folder / project). Submitting dispatches the typed prompt to your **default agent**, scoped to the current doc (or, in a folder view, the folder), which the agent receives as an `@`-mention. Until you pick something else, that default is an **In app** agent, so the conversation stays inside OpenKnowledge; it falls back to a **Terminal** CLI — preferring one detected on this machine — and then to an **External app**, when no higher option is available here. The agent picker beside the field overrides the default per send, listing In app agents first, then the [Terminal](#open-with-ai) CLIs available on this machine, then the external agent apps detected here. Whatever you pick is remembered on that machine and stays your default until you change it or turn it off in **Agent connections**. Success and failure use the same handoff toast as Open with AI.

The field also takes attachments when an **In app** agent is selected: drop a file on it, paste a screenshot into it, or use the attach button beside the field. Each one appears as a removable chip above the input and travels with the prompt into the new conversation. Images can come from anywhere; other files are sent as a path reference, so they have to live inside the project. Images must be PNG, JPEG, GIF, or WebP, and all attachments on one message are capped at 700 KB combined. That budget is tight for screenshots — a full-screen Retina capture is usually several megabytes and will be refused, so crop the capture or resize it first. Pick a **Terminal** CLI or an **External app** instead and the composer refuses the drop with the reason — those targets have no attachment channel — and if you attach first and switch afterwards, the send is blocked with the same explanation rather than silently dropping the file. Attachments aren't available in note windows yet, and whether an in-app agent can actually read an image is up to that agent: if it can't, OpenKnowledge shows a notification saying so once the agent's capabilities are known. The empty-state create composer on a fresh project takes attachments the same way — drop, paste, or the attach button while an **In app** agent is picked — and refuses them with the reason for **Terminal** and **External apps** targets.

The composer stays out of the way where it doesn't belong: it's hidden inside an AI editor's embedded view, while the [docked terminal](#terminal) is open (the terminal is its own AI entry point), and when no doc or folder is open (the empty state's create composer offers the same handoff, attachments included). It's also dismissible — an **Ask AI** badge in the status bar brings it back.
...
Expand all and Collapse all only appear when they have work to do: Expand all is hidden when no folder is collapsed; Collapse all is hidden when no folder is expanded. New from template appears when templates are available for the target folder, and also whenever its folder config has not loaded — if the load failed, the item opens to a disabled "Couldn't load templates" row. A symlink, a plain file, or an unreadable directory under `.ok/` can drop a folder's templates (hiding the item); a symlink at the folder's own `.ok` fails its config load (the disabled row) — see [where templates are stored](../advanced/folders-and-templates.mdx#where-its-stored). Share appears only when the project has a GitHub remote.
...
A `tag:` prefix searches by tag: `tag:` alone lists every tag, `tag:fr` narrows the tag list, and `tag:frontend` lists the docs tagged `#frontend` (including its hierarchy children). When semantic search is enabled and the endpoint has either a stored key or a loopback address that needs none, a **By meaning** pill switches the palette to meaning-based search. Press Enter to run it; typing alone never fires a query.
